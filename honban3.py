...
if st.session_state.page == "home":
    ...
    if st.session_state.start_button_clicked:
        ...
        st.markdown("""
                    ...
                    タスクは以下の順番で進めます。<br>
                    1. それぞれのページで「問題スタート」ボタンをクリックすると、対象となる生徒のプロフィールが表示されます。<br>
                    2. 成績の予想をする前に、あなたが重要だと思うプロフィール情報を1つ選びましょう。<br>
                    3. AIがその情報を重要視した予想を教えてくれます(教えてくれない場合もあります)。ただし、AIの予想は必ずしも正しいわけではありません。<br>
                    4. 2と3の操作は何回でも行うことができます。いろいろな情報に基づくAIの予想を聞いて、参考にしてみてください。<br>
                    5. 成績の予想ができたら、該当する成績のボタンを選びます。なお、成績は「平均より優れている」または「平均以下」の２つから選びます。<br>
                    6. 最後に提出ボタンを押してください。<br><br>
        ...
        image_path = "./intro_1_propose.jpg"  # ローカルの画像ファイルパス
        st.image(image_path, caption="問題を解く画面の例その1")
        image_path = "./intro_2_propose.jpg"  # ローカルの画像ファイルパス
        st.image(image_path, caption="問題を解く画面の例その2")
        st.markdown("<hr>", unsafe_allow_html=True)

        st.markdown('準備ができたら、下の「準備ができました」ボタンを押してください。練習問題(5問)が始まります。<br>【重要】画面の不具合がありましたら、右上の黒い点が3つあるボタンから「Rerun」を押してください。<br>この先でURLをリロードすると最初からやり直しになります。')
    ...
    if st.button("問題スタート"):
        ...
        st.session_state.select_feature = []
    ...
    if st.session_state.start_button_clicked_t1p1:
        ...
        df_high_preds = pd.read_csv('./preds/high_preds_q9.csv')
        y_pred = df_high_preds.iloc[:,0:1].values.flatten().tolist()

        # 特徴量のデータフレーム
        df_feature = pd.read_csv('./importance/high_feature_q9.csv')
        df_feature = df_feature.iloc[:-1,0:1]

        #checkboxes = [] 
        #for name in df["内容"]:
        #    checkbox = st.checkbox(f"{name}", key=name+"q1")
        #    checkboxes.append(checkbox)

        selected_option = st.radio(
            " ", 
            ['重要だと思う情報を以下から選んでください', '家から学校までの通学時間', '毎週の勉強時間', '補習などの教育サポート', '学習塾など有料の教育サポート', '友人と出かける頻度', '学校を休んだ回数'],
            index=0,
            key="t1q1"
        )
        ...
        # 選択の数をチェック
        if selected_option == "重要だと思う情報を以下から選んでください":
            st.warning("プロフィール情報を1つ選択してください。")
            st.session_state.human_prediction_finished = False
        else:
            st.session_state.selected_option = selected_option
            st.success(f"選択された項目: {st.session_state.selected_option}")

        if st.button('AIの意見を聞く'):

            if st.session_state.selected_option == "重要だと思う情報を以下から選んでください":
                st.warning("プロフィール情報を1つ選択してください。")
            else:
                st.session_state.select_count += 1
                st.session_state.select_feature.append(st.session_state.selected_option)
                # 各行とリストが一致しているかを判定
                ...
                if count > 0:
                    ...
                    if pred >= 0.5:
                        ...
                        AI_result = "平均以下"
                    st.markdown("#### AIの意見：<br>その情報に着目すると、成績は「"+AI_result+"」です。", unsafe_allow_html=True)
                else:
                    st.markdown("#### AIはその情報にあまり着目する必要はないと考えています。", unsafe_allow_html=True)
        ...
        if st.session_state.human_prediction_finished:
            ...
            with form:
                text1 = st.selectbox("生徒の成績はどちらだと思いますか？", ["選択してください", "平均より優れている", "平均以下"])
                ...
                if text1 == "選択してください":
                    ...
                    while retries <= max_retries:
                        ...
                        try:
                            ...
                            worksheet.update_acell('A12', str(st.session_state.select_feature))
    ...
    if st.session_state.start_button_clicked_t1p2:
        ...
        df_high_preds = pd.read_csv('./preds/high_preds_q15.csv')
        y_pred = df_high_preds.iloc[:,0:1].values.flatten().tolist()
        # 特徴量のデータフレーム
        df_feature = pd.read_csv('./importance/high_feature_q15.csv')
        df_feature = df_feature.iloc[:-1,0:1]
                ...
                if text1 == "選択してください":
                    ...
                    while retries <= max_retries:
                        ...
                        try:
                            ...
                            worksheet.update_acell('B12', str(st.session_state.select_feature))
    ...
    if st.session_state.start_button_clicked_t1p3:
        ...
        # DataFrameを表示
        st.markdown(df.style.hide(axis="index").to_html(), unsafe_allow_html=True)
        df_high_preds = pd.read_csv('./preds/high_preds_q25.csv')
        y_pred = df_high_preds.iloc[:,0:1].values.flatten().tolist()

        # 特徴量のデータフレーム
        df_feature = pd.read_csv('./importance/high_feature_q25.csv')
        df_feature = df_feature.iloc[:-1,0:1]
                ...
                if text1 == "選択してください":
                    ...
                    while retries <= max_retries:
                        ...
                        try:
                            ...
                            worksheet.update_acell('C12', str(st.session_state.select_feature))
    ...
    if st.session_state.start_button_clicked_t1p4:
        ...
        df_high_preds = pd.read_csv('./preds/high_preds_q44.csv')
        y_pred = df_high_preds.iloc[:,0:1].values.flatten().tolist()

        # 特徴量のデータフレーム
        df_feature = pd.read_csv('./importance/high_feature_q44.csv')
        df_feature = df_feature.iloc[:-1,0:1]
                ...
                if text1 == "選択してください":
                    ...
                    while retries <= max_retries:
                        ...
                        try:
                            ...
                            worksheet.update_acell('D12', str(st.session_state.select_feature))
    ...
    if st.session_state.start_button_clicked_t1p5:
        ...
        df_high_preds = pd.read_csv('./preds/high_preds_q47.csv')
        y_pred = df_high_preds.iloc[:,0:1].values.flatten().tolist()

        # 特徴量のデータフレーム
        df_feature = pd.read_csv('./importance/high_feature_q47.csv')
        df_feature = df_feature.iloc[:-1,0:1]
                ...
                if text1 == "選択してください":
                    ...
                    while retries <= max_retries:
                        ...
                        try:
                            ...
                            worksheet.update_acell('E12', str(st.session_state.select_feature))
    ...
    if st.session_state.start_button_clicked_t2p1:
        ...
        df_high_preds = pd.read_csv('./preds/high_preds_q2.csv')
        y_pred = df_high_preds.iloc[:,0:1].values.flatten().tolist()

        # 特徴量のデータフレーム
        df_feature = pd.read_csv('./importance/high_feature_q2.csv')
        df_feature = df_feature.iloc[:-1,0:1]
                ...
                if text1 == "選択してください":
                    ...
                    while retries <= max_retries:
                        ...
                        try:
                            ...
                            worksheet.update_acell('A22', str(st.session_state.select_feature))
    ...
    if st.session_state.start_button_clicked_t2p2:
        ...
        df_high_preds = pd.read_csv('./preds/high_preds_q3.csv')
        y_pred = df_high_preds.iloc[:,0:1].values.flatten().tolist()

        # 特徴量のデータフレーム
        df_feature = pd.read_csv('./importance/high_feature_q3.csv')
        df_feature = df_feature.iloc[:-1,0:1]
                ...
                if text1 == "選択してください":
                    ...
                    while retries <= max_retries:
                        ...
                        try:
                            ...
                            worksheet.update_acell('B22', str(st.session_state.select_feature))
    ...
    if st.session_state.start_button_clicked_t2p3:
        ...
        df_high_preds = pd.read_csv('./preds/high_preds_q7.csv')
        y_pred = df_high_preds.iloc[:,0:1].values.flatten().tolist()

        # 特徴量のデータフレーム
        df_feature = pd.read_csv('./importance/high_feature_q7.csv')
        df_feature = df_feature.iloc[:-1,0:1]
                ...
                if text1 == "選択してください":
                    ...
                    while retries <= max_retries:
                        ...
                        try:
                            ...
                            worksheet.update_acell('C22', str(st.session_state.select_feature))
    ...
    if st.session_state.start_button_clicked_t2p4:
        ...
        df_high_preds = pd.read_csv('./preds/high_preds_q8.csv')
        y_pred = df_high_preds.iloc[:,0:1].values.flatten().tolist()

        # 特徴量のデータフレーム
        df_feature = pd.read_csv('./importance/high_feature_q8.csv')
        df_feature = df_feature.iloc[:-1,0:1]
                ...
                if text1 == "選択してください":
                    ...
                    while retries <= max_retries:
                        ...
                        try:
                            ...
                            worksheet.update_acell('D22', str(st.session_state.select_feature))
    ...
    if st.session_state.start_button_clicked_t2p5:
        ...
        df_high_preds = pd.read_csv('./preds/high_preds_q17.csv')
        y_pred = df_high_preds.iloc[:,0:1].values.flatten().tolist()

        # 特徴量のデータフレーム
        df_feature = pd.read_csv('./importance/high_feature_q17.csv')
        df_feature = df_feature.iloc[:-1,0:1]
                ...
                if text1 == "選択してください":
                    ...
                    while retries <= max_retries:
                        ...
                        try:
                            ...
                            worksheet.update_acell('E22', str(st.session_state.select_feature))
    ...
    if st.session_state.start_button_clicked_t2p6:
        ...
        df_high_preds = pd.read_csv('./preds/high_preds_q12.csv')
        y_pred = df_high_preds.iloc[:,0:1].values.flatten().tolist()

        # 特徴量のデータフレーム
        df_feature = pd.read_csv('./importance/high_feature_q12.csv')
        df_feature = df_feature.iloc[:-1,0:1]
    ...
    if st.session_state.start_button_clicked_t2p7:
        ...
        df_high_preds = pd.read_csv('./preds/high_preds_q18.csv')
        y_pred = df_high_preds.iloc[:,0:1].values.flatten().tolist()

        # 特徴量のデータフレーム
        df_feature = pd.read_csv('./importance/high_feature_q18.csv')
        df_feature = df_feature.iloc[:-1,0:1]
                ...
                if text1 == "選択してください":
                    ...
                    while retries <= max_retries:
                        ...
                        try:
                            ...
                            worksheet.update_acell('G22', str(st.session_state.select_feature))
    ...
    if st.session_state.start_button_clicked_t2p8:
        ...
        df_high_preds = pd.read_csv('./preds/high_preds_q21.csv')
        y_pred = df_high_preds.iloc[:,0:1].values.flatten().tolist()

        # 特徴量のデータフレーム
        df_feature = pd.read_csv('./importance/high_feature_q21.csv')
        df_feature = df_feature.iloc[:-1,0:1]
                ...
                if text1 == "選択してください":
                    ...
                    while retries <= max_retries:
                        ...
                        try:
                            ...
                            worksheet.update_acell('H22', str(st.session_state.select_feature))
    ...
    if st.session_state.start_button_clicked_t2p9:
        ...
        df_high_preds = pd.read_csv('./preds/high_preds_q24.csv')
        y_pred = df_high_preds.iloc[:,0:1].values.flatten().tolist()

        # 特徴量のデータフレーム
        df_feature = pd.read_csv('./importance/high_feature_q24.csv')
        df_feature = df_feature.iloc[:-1,0:1]
                ...
                if text1 == "選択してください":
                    ...
                    while retries <= max_retries:
                        ...
                        try:
                            ...
                            worksheet.update_acell('I22', str(st.session_state.select_feature))
    ...
    if st.session_state.start_button_clicked_t2p10:
        ...
        df_high_preds = pd.read_csv('./preds/high_preds_q37.csv')
        y_pred = df_high_preds.iloc[:,0:1].values.flatten().tolist()

        # 特徴量のデータフレーム
        df_feature = pd.read_csv('./importance/high_feature_q37.csv')
        df_feature = df_feature.iloc[:-1,0:1]
                ...
                if text1 == "選択してください":
                    ...
                    while retries <= max_retries:
                        ...
                        try:
                            ...
                            worksheet.update_acell('J22', str(st.session_state.select_feature))
    ...
    if st.session_state.start_button_clicked_t2p11:
        ...
        df_high_preds = pd.read_csv('./preds/high_preds_q10.csv')
        y_pred = df_high_preds.iloc[:,0:1].values.flatten().tolist()

        # 特徴量のデータフレーム
        df_feature = pd.read_csv('./importance/high_feature_q10.csv')
        df_feature = df_feature.iloc[:-1,0:1]
                ...
                if text1 == "選択してください":
                    ...
                    while retries <= max_retries:
                        ...
                        try:
                            ...
                            worksheet.update_acell('K22', str(st.session_state.select_feature))
    ...
    if st.session_state.start_button_clicked_t2p12:
        ...
        df_high_preds = pd.read_csv('./preds/high_preds_q38.csv')
        y_pred = df_high_preds.iloc[:,0:1].values.flatten().tolist()

        # 特徴量のデータフレーム
        df_feature = pd.read_csv('./importance/high_feature_q38.csv')
        df_feature = df_feature.iloc[:-1,0:1]
                ...
                if text1 == "選択してください":
                    ...
                    while retries <= max_retries:
                        ...
                        try:
                            ...
                            worksheet.update_acell('L22', str(st.session_state.select_feature))
    ...
    if st.session_state.start_button_clicked_t2p13:
        ...
        df_high_preds = pd.read_csv('./preds/high_preds_q45.csv')
        y_pred = df_high_preds.iloc[:,0:1].values.flatten().tolist()

        # 特徴量のデータフレーム
        df_feature = pd.read_csv('./importance/high_feature_q45.csv')
        df_feature = df_feature.iloc[:-1,0:1]
                ...
                if text1 == "選択してください":
                    ...
                    while retries <= max_retries:
                        ...
                        try:
                            ...
                            worksheet.update_acell('M22', str(st.session_state.select_feature))
    ...
    if st.session_state.start_button_clicked_t2p14:
        ...
        df_high_preds = pd.read_csv('./preds/high_preds_q46.csv')
        y_pred = df_high_preds.iloc[:,0:1].values.flatten().tolist()

        # 特徴量のデータフレーム
        df_feature = pd.read_csv('./importance/high_feature_q46.csv')
        df_feature = df_feature.iloc[:-1,0:1]
                ...
                if text1 == "選択してください":
                    ...
                    while retries <= max_retries:
                        ...
                        try:
                            ...
                            worksheet.update_acell('N22', str(st.session_state.select_feature))
    ...
    if st.session_state.start_button_clicked_t2p15:
        ...
        df_high_preds = pd.read_csv('./preds/high_preds_q49.csv')
        y_pred = df_high_preds.iloc[:,0:1].values.flatten().tolist()

        # 特徴量のデータフレーム
        df_feature = pd.read_csv('./importance/high_feature_q49.csv')
        df_feature = df_feature.iloc[:-1,0:1]
                ...
                if text1 == "選択してください":
                    ...
                    while retries <= max_retries:
                        ...
                        try:
                            ...
                            worksheet.update_acell('O22', str(st.session_state.select_feature))
